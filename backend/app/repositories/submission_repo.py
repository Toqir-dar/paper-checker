...
class SubmissionRepository:
    ...
    async def get(self, submission_id: str) -> Submission | None:
        doc = await self._collection.find_one({"_id": ObjectId(submission_id), "user_id": current_user_id()})
        if doc is None:
            return None
        doc["_id"] = str(doc["_id"])
    ...
    async def list_by_answer_key(self, answer_key_id: str) -> list[Submission]:
        cursor = self._collection.find({"answer_key_id": answer_key_id, "user_id": current_user_id()})
        results = []
    ...
    async def list_by_batch(self, batch_id: str) -> list[Submission]:
        cursor = self._collection.find({"batch_id": batch_id, "user_id": current_user_id()})
        results = []
        async for doc in cursor:
            doc["_id"] = str(doc["_id"])
            results.append(Submission.model_validate(doc))
    ...
    async def delete(self, submission_id: str) -> bool:
        result = await self._collection.delete_one({"_id": ObjectId(submission_id), "user_id": current_user_id()})
    ...
    async def delete_by_batch(self, batch_id: str) -> int:
        ...
        result = await self._collection.delete_many({"batch_id": batch_id, "user_id": current_user_id()})
        return result.deleted_count
