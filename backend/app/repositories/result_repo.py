...
class ResultRepository:
    ...
    async def upsert(self, result: GradeResult) -> GradeResult:
        ...
        existing = await self._collection.find_one({"submission_id": result.submission_id, "user_id": current_user_id()})
    ...
    async def get_by_submission(self, submission_id: str) -> GradeResult | None:
        doc = await self._collection.find_one({"submission_id": submission_id, "user_id": current_user_id()})
        if doc is None:
            return None
        doc["_id"] = str(doc["_id"])
    ...
    async def get(self, result_id: str) -> GradeResult | None:
        doc = await self._collection.find_one({"_id": ObjectId(result_id), "user_id": current_user_id()})
        if doc is None:
            return None
        doc["_id"] = str(doc["_id"])
        return GradeResult.model_validate(doc)
    ...
    async def delete_by_submission(self, submission_id: str) -> bool:
        result = await self._collection.delete_one({"submission_id": submission_id, "user_id": current_user_id()})
    ...
    async def delete_by_submission_ids(self, submission_ids: list[str]) -> int:
        ...
        if not submission_ids:
            return 0
        result = await self._collection.delete_many({"submission_id": {"$in": submission_ids}, "user_id": current_user_id()})
        return result.deleted_count
